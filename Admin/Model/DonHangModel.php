...
class DonHangModel {
      ...
      public function getAll() {
            $getAllQuery = "SELECT * FROM donhang ORDER BY MaDH DESC";
      ...
      public function getAllByTrangThai($TrangThai) {
            $getAllByTrangThaiQuery = "SELECT * FROM donhang WHERE donhang.TrangThaiDH = '$TrangThai' ORDER BY MaDH DESC";
            $result = $this->db->select($getAllByTrangThaiQuery);
            if(count($result) > 0){
                  return $result;
            }
            return null;
      }
      public function getUserByMaDH($MaDH) {
            $getUserByMaDHQuery = "SELECT * FROM donhang, taikhoan WHERE donhang.MaTK = taikhoan.MaTK and donhang.MaDH = '$MaDH'";
            $result = $this->db->select($getUserByMaDHQuery);
            if(count($result) > 0){
                  return $result[0];
            }
            return null;
      ...
      public function getChiTietDonHang($MaDH) {
            $getChiTietDonHangQuery = "SELECT sp.AnhSP, sp.TenSP, ctdh.GiaLucMua, ctdh.SoLuong,ctdh.ThanhTien, dh.TongThanhToan FROM donhang dh, sanpham sp, chitietdonhang ctdh WHERE dh.MaDH = ctdh.MaDH and ctdh.MaSP = sp.MaSP and dh.MaDH = '$MaDH'";
      ...
      public function themDonHang($MaKM, $MaTK, $MaDC, $TongGiaTri) {
            ...
            if($MaKM == NULL) {
                  $themDonHangQuery = "INSERT INTO `donhang` (`MaDH`, `MaKM`, `MaTK`, `MaDC`, `TongGiaTri`, `NgayDatHang`, `TrangThaiDH`) VALUES ('".$this->createNewMaDH()."', 'NULL', '$MaTK', '$MaDC', '$TongGiaTri', 'Now()', 'Chưa Xử Lý');";
            } else {
                  $themDonHangQuery = "INSERT INTO `donhang` (`MaDH`, `MaKM`, `MaTK`, `MaDC`, `TongGiaTri`, `NgayDatHang`, `TrangThaiDH`) VALUES ('".$this->createNewMaDH()."', '$MaKM', '$MaTK', '$MaDC', '$TongGiaTri', 'Now()', 'Chưa Xử Lý');";
            }
